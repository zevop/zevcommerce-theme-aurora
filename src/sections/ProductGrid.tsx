...
export default function ProductGrid({ settings, blocks }: { settings: any; blocks: any[] }) {
  ...
  const gridColsMap: Record<string, string> = { '2': 'lg:grid-cols-2', '3': 'lg:grid-cols-3', '4': 'lg:grid-cols-4', '5': 'lg:grid-cols-5' };
  const gridCols = gridColsMap[columns] || 'lg:grid-cols-4';

  if (loading) {
    return (
      <div className="container mx-auto px-4 sm:px-6" style={{ paddingTop, paddingBottom }}>
        <div className="animate-pulse space-y-8">
          <div className="h-8 bg-stone-200 w-1/3 rounded" />
          <div className={`grid grid-cols-2 md:grid-cols-3 ${gridCols} gap-6`}>
            {[...Array(parseInt(columns))].map((_, i) => <div key={i} className="aspect-[3/4] bg-stone-100 rounded-xl" />)}
          </div>
        </div>
      </div>
    );
  ...
  return (
    <section style={{ paddingTop, paddingBottom, backgroundColor: 'var(--color-background)' }}>
      <div className="container mx-auto px-4 sm:px-6">
        {headingBlock && (
          <div className="mb-10">
            <BlockRenderer blocks={[headingBlock]} sectionSettings={settings} />
          </div>
        )}
        <div className={`grid grid-cols-2 md:grid-cols-3 ${gridCols} gap-5 md:gap-7`}>
          {products.map(product => (
            <ProductCard key={product.id} product={product} domain={domain} aspectRatio={imageRatio} borderRadius="lg" />
          ))}
        </div>
      </div>
    </section>
...
export const schema = {
  ...
  settings: [
    ...
    { type: 'select', id: 'image_ratio', label: 'Image Ratio', options: [{ value: 'auto', label: 'Auto' }, { value: 'square', label: 'Square' }, { value: 'portrait', label: 'Portrait' }, { value: 'landscape', label: 'Landscape' }], default: 'portrait' },
  ],
  blocks: getSharedBlocks([
    { type: 'heading', name: 'Heading', settings: [{ type: 'text', id: 'text', label: 'Heading', default: 'Featured Collection' }, { type: 'select', id: 'size', label: 'Size', options: [{ value: 'sm', label: 'Small' }, { value: 'lg', label: 'Large' }], default: 'lg' }, { type: 'select', id: 'alignment', label: 'Alignment', options: [{ value: 'left', label: 'Left' }, { value: 'center', label: 'Center' }], default: 'left' }] },
    { type: 'product_grid_items', name: 'Product Grid', settings: [{ type: 'range', id: 'limit', label: 'Products', min: 4, max: 24, step: 4, default: 8 }, { type: 'select', id: 'columns_desktop', label: 'Columns', options: [{ value: '2', label: '2' }, { value: '3', label: '3' }, { value: '4', label: '4' }], default: '4' }] },
  ]),
};
